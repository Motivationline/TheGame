...
(function (Script) {
    var ƒ = FudgeCore;
    Script.availableJobs = new Set([Script.JobType.NONE, Script.JobType.GATHER_FOOD, Script.JobType.GATHER_STONE]);
    let uis;
    let activeUI;
    function setupUI() {
        if (ƒ.Project.mode === ƒ.MODE.EDITOR)
                    ...
                    switch (this.#progress) {
                        ...
                        case 6: {
                            ...
                            if (this.#job === Script.JobType.GATHER_FOOD) {
                                Script.Data.food += Math.max(1, Math.floor(1 * Script.Data.gatherBonusFood));
                            }
                            else if (this.#job === Script.JobType.GATHER_STONE) {
                                Script.Data.stone += Math.max(1, Math.floor(1 * Script.Data.gatherBonusStone));
                            }
                            this.#progress = 0;
                            break;
                        }
                ...
                this.idle = (deltaTime) => {
                    switch (this.#progress) {
                        ...
                        case 2: {
                            ...
                            node.mtxLocal.translateX(Math.max(-20, Math.min(20, Script.randomRange(-4, 4) + this.node.mtxWorld.translation.x)));
                            node.mtxLocal.translateZ(Math.max(-20, Math.min(20, Script.randomRange(-4, 4) + this.node.mtxWorld.translation.z)));
                            this.#target = jp;
